...
function search(){
    let inputSearch = document.getElementById("search-box").value;
    let data = document.getElementById("data");
    inputSearch = inputSearch.replace(/[.**?^${}() | [\]\\]/gi,"\\$&");
    let pattern = new RegExp(`${inputSearch}`, "gi");
    data.innerHTML = data.textContent.replace(pattern, match => `<mark>${match}</mark>`)
}
